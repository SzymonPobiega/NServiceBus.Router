namespace NServiceBus.Router
{
    ...
    public class SettingsHolder : IReadOnlySettings
    {
        ...
        internal void Merge(SettingsHolder other)
        {
            Guard.AgainstNull(nameof(other), other);

            EnsureMergingIsPossible();

            foreach (var @default in other.Defaults)
            {
                Defaults[@default.Key] = @default.Value;
            }

            foreach (var @override in other.Overrides)
            {
