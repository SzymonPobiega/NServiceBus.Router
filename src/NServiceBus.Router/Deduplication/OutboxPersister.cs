...
namespace NServiceBus.Router.Deduplication
{
    class OutboxPersister
    {
        ...
        public async Task Install(string receiverSequenceKey, SqlConnection conn, SqlTransaction trans)
        {
            ...
            await CreateOutboxTable(lo, conn, trans).ConfigureAwait(false);
            await CreateConstraint(lo, 0, epochSize, conn, trans).ConfigureAwait(false);
            await CreateOutboxTable(hi, conn, trans).ConfigureAwait(false);
        ...
        public async Task Uninstall(string receiverSequenceKey, SqlConnection conn, SqlTransaction trans)
        {
            var lo = $"Outbox_{sourceSequenceKey}_{receiverSequenceKey}_1";
            var hi = $"Outbox_{sourceSequenceKey}_{receiverSequenceKey}_2";

            await DropWatermarksTable(conn, trans).ConfigureAwait(false);

            await DropOutboxTable(lo, conn, trans).ConfigureAwait(false);
            await DropOutboxTable(hi, conn, trans).ConfigureAwait(false);

            await DropSequence($"Sequence_{sourceSequenceKey}_{receiverSequenceKey}", conn, trans).ConfigureAwait(false);
        ...
        Task DropWatermarksTable(SqlConnection conn, SqlTransaction trans)
        {
            var script = $@"
IF EXISTS (
    SELECT *
...
";
        ...
        static Task DropOutboxTable(string name, SqlConnection conn, SqlTransaction trans)
        {
            var script = $@"
IF EXISTS (
    SELECT *
...
";
        ...
        static Task CreateOutboxTable(string name, SqlConnection conn, SqlTransaction trans)
        {
            var script = $@"
IF EXISTS (
    SELECT *
...
";
        ...
        Task DropSequence(string name, SqlConnection conn, SqlTransaction trans)
        {
            var script = $@"
IF EXISTS (
    SELECT * 
