...
namespace NServiceBus.Router.Deduplication
{
    class InboxPersister
    {
        ...
        public async Task Install(string sourceSequenceKey, SqlConnection conn, SqlTransaction trans)
        {
            ...
            await CreateInboxTable(lo, conn, trans).ConfigureAwait(false);
            await CreateConstraint(lo, 0, epochSize, conn, trans).ConfigureAwait(false);
            await CreateInboxTable(hi, conn, trans).ConfigureAwait(false);
        ...
        public async Task Uninstall(string sourceSequenceKey, SqlConnection conn, SqlTransaction trans)
        {
            ...
            await DropInboxTable(lo, conn, trans).ConfigureAwait(false);
            await DropInboxTable(hi, conn, trans).ConfigureAwait(false);
        }
...
";
        ...
        static Task DropInboxTable(string name, SqlConnection conn, SqlTransaction trans)
        {
            var script = $@"
IF EXISTS (
    SELECT *
...
";
        ...
        static Task CreateInboxTable(string name, SqlConnection conn, SqlTransaction trans)
        {
            var script = $@"
IF EXISTS (
    SELECT *
