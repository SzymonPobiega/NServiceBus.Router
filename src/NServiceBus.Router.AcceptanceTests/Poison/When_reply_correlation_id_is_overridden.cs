...
namespace NServiceBus.Router.AcceptanceTests.Poison
{
    ...
    [TestFixture]
    public class When_reply_correlation_id_is_overridden : NServiceBusAcceptanceTest
    {
        [Test]
        public async Task Should_move_message_to_poison_queue()
        {
            ...
            Assert.AreEqual("Cannot decode value in \'NServiceBus.CorrelationId\' header: Expected type followed by a delimiter, found \'MyCorrelation\'", result.ExceptionMessage);
        ...
        class Receiver : EndpointConfigurationBuilder
        {
            public Receiver()
            {
                EndpointSetup<DefaultServer>(c =>
                {
                    ...
                    c.Pipeline.Register(new OverrideCorrelationIdBehavior(), "Removes correlation header");
                });
            }
            ...
            class OverrideCorrelationIdBehavior : IBehavior<IDispatchContext, IDispatchContext>
            {
                public Task Invoke(IDispatchContext context, Func<IDispatchContext, Task> next)
                {
                    foreach (var operation in context.Operations)
                    {
                        operation.Message.Headers[Headers.CorrelationId] = "MyCorrelation";
                    }
