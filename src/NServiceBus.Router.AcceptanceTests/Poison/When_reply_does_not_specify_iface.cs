...
namespace NServiceBus.Router.AcceptanceTests.Poison
{
    ...
    [TestFixture]
    public class When_reply_does_not_specify_iface : NServiceBusAcceptanceTest
    {
        [Test]
        public async Task Should_move_message_to_poison_queue()
        {
            ...
            Assert.AreEqual("The reply message does not contain \'iface\' correlation parameter required to route the message.", result.ExceptionMessage);
        ...
        class Receiver : EndpointConfigurationBuilder
        {
            public Receiver()
            {
                EndpointSetup<DefaultServer>(c =>
                {
                    ...
                    c.Pipeline.Register(new RemoveIfaceFromCorrelationIdBehavior(), "Removes correlation header");
                });
            }
            ...
            class RemoveIfaceFromCorrelationIdBehavior : IBehavior<IDispatchContext, IDispatchContext>
            {
